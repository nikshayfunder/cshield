...
pub async fn middleware(
    ...
    let ip = if let Some(h) = req.headers().get("x-forwarded-for") {
        ...
        // Fallback for Axum without ConnectInfo extracted in this specific middleware signature
        // Ideally: req.extensions().get::<ConnectInfo<SocketAddr>>().map(|ci| ci.0.ip().to_string()).unwrap_or_default()
        // We will assume XFF or Real-IP is set by L4 ingress or use "127.0.0.1" for dev if missing.
        "127.0.0.1".to_string()
    };
    ...
    tokio::spawn(async move {
        store.record_ip(&ip_clone).await;
    });
    ...
    if host_no_port.parse::<std::net::IpAddr>().is_ok() {
        metrics.add_attack(crate::internal::analytics::AttackEvent {
            ...
            path: path.clone(),
            method: method.clone(),
            ua: req.headers().get("user-agent").and_then(|h| h.to_str().ok()).unwrap_or("").to_string(),
    ...
    if cvac_enabled {
        ...
        let cookie_vec: Vec<String> = req.headers().get("cookie")
            ...
            .unwrap_or_default();
        let ua = req.headers().get("user-agent").and_then(|h| h.to_str().ok()).unwrap_or("").to_string();
        ...
        let ja4 = req.headers().get("x-ja4-fingerprint").and_then(|h| h.to_str().ok()).map(|s| s.to_string());
        // For cipher, we usually get it from the TLS terminator via header
        let cipher_suite = req.headers().get("x-tls-cipher-id")
    ...
    if waf_enabled {
        let headers_map = req.headers().iter().map(|(k,v)| (k.to_string(), v.to_str().unwrap_or("").to_string())).collect();
        let (parts, body) = req.into_parts();
        let bytes = axum::body::to_bytes(body, 2_000_000).await.unwrap_or_default();
        let body_len = bytes.len();
        
        let decision = state.waf.inspect(&parts.uri.to_string(), parts.method.as_str(), &headers_map, body_len);
...
fn allow_path(p: &str) -> bool {
    ...
    if p == "/v1/api/login" || p == "/login" { return true; }
    if p.starts_with("/v1/api/captcha/") { return true; }
    if ["/dashboard.html", "/settings.html", "/add_proxy.html", "/proxies.html", "/dashboard", "/settings", "/add_proxy", "/proxies", "/login.html", "/captcha.html"].contains(&p) { return true; }
