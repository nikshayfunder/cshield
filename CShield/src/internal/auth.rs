    ...
    if let Some(coll) = &state.user_coll {
        if let Ok(Some(user)) = coll.find_one(doc! { "username": &payload.username }, None).await {
            if verify_hash(&payload.password, &user.password_hash) {
                ...
                headers.insert("Set-Cookie", format!("cshield_p={}; Path=/; HttpOnly; SameSite=Lax", raw).parse().unwrap());
                headers.insert("Set-Cookie", format!("cshield_v={}; Path=/; HttpOnly; SameSite=Lax", sig).parse().unwrap());
...
pub async fn register_handler(
    ...
    let bytes = axum::body::to_bytes(body, 1024 * 10).await.unwrap_or_default();
