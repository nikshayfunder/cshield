...
async fn main() -> anyhow::Result<()> {
    ...
    let (ip_tx, mut ip_rx) = tokio::sync::mpsc::unbounded_channel::<String>();

    let store_clone = store.clone();
    tokio::spawn(async move {
        let mut batch = Vec::with_capacity(100);
        let mut interval = tokio::time::interval(std::time::Duration::from_secs(5)); // flush every 5s
        ...
        loop {
            tokio::select! {
                Some(ip) = ip_rx.recv() => {
                    batch.push(ip);
                    if batch.len() >= 100 {
                        // Optimally we would use SADD with multiple members, but record_ip takes one.
                        // We can optimize record_ip later to take &[String].
                        // For now, at least we moved it off the hot path.
                        for ip in &batch {
                            store_clone.record_ip(ip).await;
                        }
                        batch.clear();
                    }
                }
    ...
    let app = Router::new()
        ...
        .route("/dashboard", get(auth::dashboard_page_handler))
        ...
        // API routes
        .route("/v1/api/metrics", get(api::metrics_handler))
        .route("/v1/api/system/health", get(api::health_handler))
        .route("/v1/api/proxies/list", get(api::list_proxies_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/proxies/delete", post(api::delete_proxy_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/proxies/reload", post(api::reload_proxies_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/proxies/add", post(api::add_proxy_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/features/get", get(api::get_features_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/features/update", post(api::update_features_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/configs/read", get(api::read_config_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/configs/save", post(api::save_config_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/maintenance/get", get(api::get_maintenance_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/v1/api/maintenance/set", post(api::set_maintenance_handler).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))

        // Page redirects (Admin only pages)
        // Since we serve static files, we can just redirect /page -> /page.html
        // But we should protect them.
        // Let's use a simple redirect handler that is protected.
        .route("/analytics", get(|| async { axum::response::Redirect::to("/analytics.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/features", get(|| async { axum::response::Redirect::to("/features.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/proxies", get(|| async { axum::response::Redirect::to("/proxies.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/add_proxy", get(|| async { axum::response::Redirect::to("/add_proxy.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/settings", get(|| async { axum::response::Redirect::to("/settings.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/actions", get(|| async { axum::response::Redirect::to("/actions.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin))) 
        .route("/users", get(|| async { axum::response::Redirect::to("/users.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
        .route("/logs", get(|| async { axum::response::Redirect::to("/logs.html") }).layer(axum::middleware::from_fn_with_state(state.clone(), auth::require_admin)))
    ...
    if app_config.tls.enable {
        let cert_pem = tokio::fs::read(&format!("{}/cert.pem", app_config.tls.cache_dir)).await?;
        let key_pem = tokio::fs::read(&format!("{}/key.pem", app_config.tls.cache_dir)).await?;
    ...
    if let Some(cfg) = config {
        ...
        match rb.send().await {
            Ok(resp) => {
                ...
                (status, headers_out, bytes).into_response()
            }
            Err(_) => StatusCode::BAD_GATEWAY.into_response()
        }
    } else {
        let service = ServeDir::new(DIR_PUBLIC);
        match service.oneshot(req).await {
            Ok(res) => res.into_response(),
            Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, format!("Static file error: {}", e)).into_response(),
        }
    }
}
